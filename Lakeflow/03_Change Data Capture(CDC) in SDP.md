...
1. エラーがない場合、パイプライングラフとテーブルが以下図のように出力されます。
</br><img src="../images/sdp.9.png" width="600"/>  
1. **パイプラインを実行**から**テーブルを完全に更新してパイプラインを実行**をクリックし実行します。
1. エラーがないことを確認し、次のラボに移ります。作成したパイプラインは、後述のLakeflow Jobsで呼び出し実行を行います。